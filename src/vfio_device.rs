...
impl fmt::Display for VfioError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            VfioError::VfioRegionInfoCapUnknown(c) => {
                write!(f, "unknown region capability: {}", c)
            }
        }
    }
}
...
impl std::error::Error for VfioError {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        match self {
            ...
            VfioError::VfioRegionInfoCapUnknown(_c) => None,
        }
    }
}
...
impl VfioDeviceInfo {
    ...
    fn get_region_map(
        ...
        if region_with_cap[0].region_info.cap_offset > region_info_size {
            ...
            let info_ptr = &region_with_cap[0] as *const vfio_region_info_with_cap;
            ...
            while next_cap_offset > region_info_size {
                ...
                match u32::from(cap_header.id) {
                    ...
                    _ => {
                        return Err(VfioError::VfioRegionInfoCapUnknown(cap_header.id));
                    }
                }
